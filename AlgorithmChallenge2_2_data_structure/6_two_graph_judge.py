...
def solve(n, chains):
    ...
    for first, second in chains:
        chain_table[first][second] = 1
        chain_table[second][first] = 1
    for edge_no, chain_row in chain_table.items():
        for _edge_no, step_sts in chain_row.items():
            if step_sts == 1:
                chain_table[edge_no][_edge_no] = 2
                chain_table[_edge_no][edge_no] = 2
...
def _check_two_graph(target_edge, steps):
    ...
    for _edge_no, step_sts in chain_table[target_edge].items():
        if step_sts == 1:
            chain_table[target_edge][_edge_no] = 2
            chain_table[_edge_no][target_edge] = 2
            steps[target_edge] = 0
